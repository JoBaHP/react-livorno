...
exports.populateStreetsFromCity = async (req, res) => {
  ...
  try {
    ...
    streetsData.elements.forEach((element) => {
      // --- FIX ---
      // Ensure that the street name exists and is not an empty string before adding it.
      if (
        element.tags &&
        element.tags.name &&
    ...
    try {
      ...
      for (const name of streetNames) {
        await client.query(
          "INSERT INTO streets (name, name_normalized) VALUES ($1, unaccent($1)) ON CONFLICT (name) DO NOTHING",
          [name]
        );
      }
