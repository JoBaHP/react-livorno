  ...
  try {
    ...
    try {
      ...
      for (const name of streetNames) {
        // --- FIX ---
        // Explicitly cast the parameter to varchar to resolve the type inconsistency.
        await client.query(
          "INSERT INTO streets (name, name_normalized) VALUES ($1::varchar, unaccent($1)) ON CONFLICT (name) DO NOTHING",
          [name]
        );
      }
...
exports.createStreet = async (req, res) => {
  ...
  try {
    const { rows } = await db.query(
      "INSERT INTO streets (name, name_normalized) VALUES ($1, unaccent($1)) RETURNING *",
      [name]
    );
