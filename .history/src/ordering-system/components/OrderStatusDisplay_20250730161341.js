...
export default function OrderStatusDisplay({ order, setOrderStatus }) {
  const getStatusInfo = () => {
    // --- FIX ---
    // Safely access the wait time, checking for both database (`wait_time`)
    // and JavaScript (`waitTime`) naming conventions.
    ...
    switch (order.status) {
      ...
      case "accepted":
        return {
          text: `Preparing. Est. wait: ${waitTimeDisplay} mins`,
          color: "text-blue-600",
          icon: <CheckCircle />,
        };
      case "preparing":
        return {
          text: `In the kitchen! Est. wait: ${waitTimeDisplay} mins`,
