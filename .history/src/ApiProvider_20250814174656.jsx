...
export const ApiProvider = ({ children }) => {
    const api = {
        ...
        getAllStreets: async (fetchAll = false) => {
            const url = fetchAll ? `${API_URL}/api/streets?all=true` : `${API_URL}/api/streets`;
        ...
        searchStreets: async (term) => {
            ...
            const response = await fetch(`${API_URL}/api/streets/search?term=${encodeURIComponent(term)}`);
            return response.json();
        },
    ...
    return <ApiContext.Provider value={api}>{children}</ApiProvider>;
};
