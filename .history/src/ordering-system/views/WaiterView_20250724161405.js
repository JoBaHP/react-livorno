...
export default function WaiterView() {
  ...
  useEffect(() => {
    // The API now returns an object with an 'orders' property.
    // We also pass a large limit, as the waiter view doesn't need pagination.
    api
      .getOrders({ limit: 100 })
      .then((result) => {
        // --- FIX ---
        // Check if the result and the orders array exist before trying to filter.
        if (result && result.orders) {
          setOrders(result.orders.filter((o) => o.status !== "completed"));
        } else {
          // If the data is not in the expected format, log an error.
