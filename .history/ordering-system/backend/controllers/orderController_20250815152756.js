...
exports.placeDeliveryOrder = async (req, res) => {
  ...
  // Make the address more specific for the geocoding API.
  const fullAddressForGeocoding = `${customerAddress}, Ulcinj, Montenegro`;

  let location;
  try {
    const response = await fetch(
      `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(
        fullAddressForGeocoding
      )}`,
      {
        headers: {
...
exports.placeDeliveryOrder = async (req, res) => {
  ...
  const geocodingAddress = `${customerDetails.street} ${customerDetails.number}, Ulcinj, Montenegro`;
