...
exports.searchStreets = async (req, res) => {
  ...
  try {
    // --- FIX ---
    // This corrected query ensures that both the database column and the
    // search term are processed by unaccent, making the search work for all character sets.
    const query =
      "SELECT * FROM streets WHERE unaccent(name) ILIKE unaccent($1) ORDER BY name ASC LIMIT 10";
    const params = [`%${term}%`];

    console.log("Executing Street Search:", query, params); // For debugging
