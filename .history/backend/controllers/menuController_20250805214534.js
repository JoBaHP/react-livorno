...
exports.updateMenuItem = async (req, res) => {
  const { id } = req.params;
  const { name, category, description, price, sizes, available, imageUrl } =
    req.body;
  const client = await db.query("BEGIN");
  try {
    const itemResult = await db.query(
    ...
    // Update options by first deleting existing ones, then adding the new selection
    await db.query("DELETE FROM menu_item_options WHERE menu_item_id = $1", [
      id,
    ]);
    if (req.body.options && req.body.options.length > 0) {
      for (const optionId of req.body.options) {
        await db.query(
          "INSERT INTO menu_item_options (menu_item_id, option_id) VALUES ($1, $2)",
          [id, optionId]
        );
