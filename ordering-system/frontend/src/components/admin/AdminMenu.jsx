...
export default function AdminMenu() {
  ...
  const normalized = useMemo(() => {
    if (Array.isArray(data)) {
      const uniqueCategories = Array.from(
        new Set(data.map((item) => (item.category != null ? item.category : null)))
      );
      return {
        items: data,
        totalPages: 1,
  ...
  const categoryOptions = useMemo(() => {
    ...
    (normalized.categories || []).forEach((cat) => {
      ...
      if (!map.has(key)) {
        map.set(key, {
          name: cat?.name ?? null,
          sortOrder: typeof cat?.sortOrder === 'number' ? cat.sortOrder : Number.MAX_SAFE_INTEGER,
        });
      }
    });
  ...
  useEffect(() => {
    ...
    const exists = categoryOptions.some((cat) => cat.value === selectedCategory);
    if (!exists) {
          ...
          <div className="bg-white w-full max-w-md rounded-lg shadow-xl p-6 mx-4">
            ...
            <div className="mt-6 flex justify-end gap-2">
              ...
              <button
                ...
                disabled={
                  reorderCategoriesMutation.isPending || categoryOrderDraft.length === 0
                }
              >
                {reorderCategoriesMutation.isPending
