...
const getDateRange = (filter) => {
  if (filter === "all") return {}; // Return no dates for the 'all' filter
  const now = new Date();
...
export default function AdminOrders() {
  ...
  const [filters, setFilters] = useState({
    activeDateFilter: "all", // Default to showing all orders
    tableId: "",
    orderId: "",
    status: "",
  });
  ...
  useEffect(() => {
    // This single useEffect now handles all data fetching triggers
...
// ... (FeedbackModal and other helpers are the same)
