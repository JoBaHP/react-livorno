...
exports.getOrders = async (req, res) => {
  ...
  const validSortColumns = ["created_at", "total", "table_id", "status"];
  ...
  try {
    ...
    // --- FIX ---
    // Create a new array for the second query's parameters to avoid mutation issues.
    const paginatedQueryParams = [...queryParams, limit, offset];
    const queryString = `SELECT * FROM orders ${whereString} ORDER BY ${orderBy} ${orderDirection} LIMIT $${paramIndex++} OFFSET $${paramIndex++}`;

    // Log the query and params to the backend console for debugging
    console.log("Executing Query:", queryString);
    console.log("With Params:", paginatedQueryParams);

    const ordersResult = await db.query(queryString, paginatedQueryParams);
