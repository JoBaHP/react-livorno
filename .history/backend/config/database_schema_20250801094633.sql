...
-- Create a table to hold all possible customization options
CREATE TABLE options (
    id SERIAL PRIMARY KEY,
    name VARCHAR(255) NOT NULL,
    price NUMERIC(10, 2) NOT NULL DEFAULT 0.00 -- Price is 0 for free items like ketchup
);

-- Create a join table to link menu items to their available options
...
-- Add new columns to the orders table for notes and payment method
ALTER TABLE orders
ADD COLUMN notes TEXT,
ADD COLUMN payment_method VARCHAR(50);

-- Add a new column to the order_items table to store selected options
ALTER TABLE order_items
ADD COLUMN selected_options JSONB;

-- (Optional) Insert some sample options to get started
