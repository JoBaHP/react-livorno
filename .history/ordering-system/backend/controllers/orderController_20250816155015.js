...
exports.placeDeliveryOrder = async (req, res) => {
  // The frontend now sends a nested customerDetails object
  const { cart, customerDetails, paymentMethod, notes } = req.body;

  // --- FIX ---
  // 1. Create a clean address specifically for the geocoding API.
  const geocodingAddress = `${customerDetails.street} ${customerDetails.number}, ${process.env.RESTAURANT_LOCATION}`;
