...
export default function OrderStatusDisplay({ order, setOrderStatus }) {
  const getStatusInfo = () => {
    switch (order.status) {
      ...
      case "accepted":
        return {
          text: `Preparing. Est. wait: ${order.wait_time} mins`,
          color: "text-blue-600",
          icon: <CheckCircle />,
        };
      case "preparing":
        return {
          text: `In the kitchen! Est. wait: ${order.wait_time} mins`,
  ...
  // --- FIX ---
  // Safely parse the total and item prices to numbers before formatting.
